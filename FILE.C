...

{
	...
	nline = 0;
	eolchar = ( curbp->b_mode & MDBINARY ) ? '\r' : '\n';
	while ((s=ffgetline(&nbytes, eolchar, &lmode)) == FIOSUC) {
...
char	*fn;
{
	...
	bflag = curbp->b_mode & MDBINARY;
	lp = lforw(curbp->b_linep);		/* First line.		*/
	nline = 0;				/* Number of lines.	*/
	while (lp != curbp->b_linep) {
		if ((s=ffputline(
			&lp->l_text[0], llength(lp), bflag, (int)lp->l_mode
		)) != FIOSUC)
			break;
		++nline;
		lp = lforw(lp);
