 ...
 *		way, encrypted files would still be just as cheap to 
 ...
 *	specifically in the case of MicroEMACS) you would use a "vector" 
...
register unsigned len;	/* number of characters in the buffer */
{
	...
	if (!bptr) {		/* is there anything here to encrypt? */
		key = salt = len;	/* set the new key and salt */
		return;
	}
...
/**  Down-bias the character, perform a Beaufort encipherment, and 
	up-bias the character again.  We want the terms to be positive 
	so that the mod 95 will be portable.	**/

			cc = ((int)(key % 95) - (cc - ' ') + 95) % 95 + ' ';
